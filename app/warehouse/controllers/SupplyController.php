...
//                        $arr['created_at'] = $this->get_by_attribute('supply','created_at',$key->fk_material);
//			$arr['qty'] = $key->qty;
//			array_push($arrdata, $arr);
//		}
        $res = $this->db->select('s.id, m.nama as material, sp.nama as supplier, s.qty, m.satuan ,s.created_at as transaction_date')
                ->from('supply as s')
                ->join('material as m', 's.fk_material = m.id')
                ->join('supplier as sp', 's.fk_supplier = sp.id');
        $get = $res->get();
        $arrdata = $get->result();        
        return $arrdata;
    }
